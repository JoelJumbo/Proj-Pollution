...
def grafico():
    ...
    #time.sleep(3)
    return stream_template("grafico.html",Permission=Permission, grafico=json_fig, comuni=json.dumps(lista_comuni))

@app.route("/richiesta_searchbar", methods=['POST', 'GET'])
def richiesta_searchbar():
    ...
    grafici.crea_json(id_stazione_vicina)
    #time.sleep(5)
...
def page_not_found(error):
    return render_template('404.html', Permission=Permission), 500
